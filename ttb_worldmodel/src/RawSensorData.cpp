...
//			pose_out.pose.orientation.z = stampedTransform.getRotation().z();
			...
			cout << "marker pos x,y,z: " << pose_out.pose.position.x << " ," << pose_out.pose.position.y << " ,"
					<< pose_out.pose.position.z << endl;
			cout << "marker orientation w,x,y,z: " << pose_out.pose.orientation.w << " ," << pose_out.pose.orientation.x
					<< " ," << pose_out.pose.orientation.y << " ," << pose_out.pose.orientation.z << endl;

			shared_ptr<geometry_msgs::PoseStamped> poseStampedDataPtr = make_shared<geometry_msgs::PoseStamped>(
					pose_out);
			shared_ptr<InformationElement<geometry_msgs::PoseStamped>> ownposeStampedInfo = make_shared<
					InformationElement<geometry_msgs::PoseStamped>>(poseStampedDataPtr, time);
	...
	void RawSensorData::processCliffEvent(kobuki_msgs::CliffEventPtr cliffEventData)
	{
		...
		shared_ptr<kobuki_msgs::CliffEvent> cliffEventPtr = shared_ptr<kobuki_msgs::CliffEvent>(
				cliffEventData.get(), [cliffEventData](kobuki_msgs::CliffEvent*) mutable
				{	cliffEventData.reset();});
		shared_ptr<InformationElement<kobuki_msgs::CliffEvent>> ownCliffEventInfo = make_shared<
				InformationElement<kobuki_msgs::CliffEvent>>(cliffEventPtr, time);
		ownCliffEvent.add(ownCliffEventInfo);
	}
	void RawSensorData::processImuData(sensor_msgs::ImuPtr imuData)
	{
		InfoTime time = wm->getTime();
		shared_ptr<sensor_msgs::Imu> imuDataPtr = shared_ptr<sensor_msgs::Imu>(imuData.get(),
																				[imuData](sensor_msgs::Imu*) mutable
																				{	imuData.reset();});
		shared_ptr<InformationElement<sensor_msgs::Imu>> ownImuDataInfo = make_shared<
				InformationElement<sensor_msgs::Imu>>(imuDataPtr, time);
...
//		cout << "Imu Ringbuffer first/last: " << ownImuData.getLast()->getInformation()->linear_acceleration << endl;
	...
	void RawSensorData::processCameraPcl(sensor_msgs::PointCloud2Ptr cameraPclData)
	{
		...
		shared_ptr<sensor_msgs::PointCloud2> cameraPclPtr = shared_ptr<sensor_msgs::PointCloud2>(
				cameraPclData.get(), [cameraPclData](sensor_msgs::PointCloud2*) mutable
				{	cameraPclData.reset();});
		shared_ptr<InformationElement<sensor_msgs::PointCloud2>> ownCameraPclInfo = make_shared<
				InformationElement<sensor_msgs::PointCloud2>>(cameraPclPtr, time);
	...
	void RawSensorData::processBumperEvents(kobuki_msgs::BumperEventPtr bumperEventsData)
	{
		...
		shared_ptr<kobuki_msgs::BumperEvent> bumperEventsDataPtr = shared_ptr<kobuki_msgs::BumperEvent>(
				bumperEventsData.get(), [bumperEventsData](kobuki_msgs::BumperEvent*) mutable
				{	bumperEventsData.reset();});
		shared_ptr<InformationElement<kobuki_msgs::BumperEvent>> ownBumperEventsInfo = make_shared<
				InformationElement<kobuki_msgs::BumperEvent>>(bumperEventsDataPtr, time);
	...
	void RawSensorData::processBumperSensors(sensor_msgs::PointCloud2Ptr bumperSensorsData)
	{
		...
		shared_ptr<sensor_msgs::PointCloud2> bumperSensorsDataPtr = shared_ptr<sensor_msgs::PointCloud2>(
				bumperSensorsData.get(), [bumperSensorsData](sensor_msgs::PointCloud2*) mutable
				{	bumperSensorsData.reset();});
		shared_ptr<InformationElement<sensor_msgs::PointCloud2>> ownBumperSensorsInfo = make_shared<
				InformationElement<sensor_msgs::PointCloud2>>(bumperSensorsDataPtr, time);
	...
	void RawSensorData::processLaserScan(sensor_msgs::LaserScanPtr laserScanData)
	{
		...
		shared_ptr<sensor_msgs::LaserScan> laserScanDataPtr = shared_ptr<sensor_msgs::LaserScan>(
				laserScanData.get(), [laserScanData](sensor_msgs::LaserScan*) mutable
				{	laserScanData.reset();});
		shared_ptr<InformationElement<sensor_msgs::LaserScan>> ownLaserScanInfo = make_shared<
				InformationElement<sensor_msgs::LaserScan>>(laserScanDataPtr, time);
	...
	void RawSensorData::processOdometryData(nav_msgs::OdometryPtr odometryData)
	{
		...
		shared_ptr<InformationElement<geometry::CNPosition>> ownPositionMotionInfo = make_shared<
...
//		ownOdom.add(ownOdomScanInfo);
	...
	void RawSensorData::processCameraImageRaw(sensor_msgs::ImagePtr cameraImageRawData)
	{
		...
		shared_ptr<sensor_msgs::Image> cameraImageRawDataPtr = shared_ptr<sensor_msgs::Image>(
				cameraImageRawData.get(), [cameraImageRawData](sensor_msgs::Image*) mutable
				{	cameraImageRawData.reset();});
		shared_ptr<InformationElement<sensor_msgs::Image>> ownCameraImageRawInfo = make_shared<
				InformationElement<sensor_msgs::Image>>(cameraImageRawDataPtr, time);
	...
	void RawSensorData::processRobotOnOff(robot_control::RobotCommandPtr robotOnOffData)
	{
		...
		shared_ptr<robot_control::RobotCommand> robotOnOffDataPtr = shared_ptr<robot_control::RobotCommand>(
				robotOnOffData.get(), [robotOnOffData](robot_control::RobotCommand*) mutable
				{	robotOnOffData.reset();});
		shared_ptr<InformationElement<robot_control::RobotCommand>> ownRobotOnOffInfo = make_shared<
	...
	void RawSensorData::processMobileBaseSensorState(kobuki_msgs::SensorStatePtr mobileBaseSensorStateData)
	{
		...
		shared_ptr<kobuki_msgs::SensorState> mobBaseSensStateDataPtr = shared_ptr<kobuki_msgs::SensorState>(
				mobileBaseSensorStateData.get(), [mobileBaseSensorStateData](kobuki_msgs::SensorState*) mutable
				{	mobileBaseSensorStateData.reset();});
		shared_ptr<InformationElement<kobuki_msgs::SensorState>> ownMobBaseSensorStateInfo = make_shared<
				InformationElement<kobuki_msgs::SensorState>>(mobBaseSensStateDataPtr, time);
	...
	void RawSensorData::processDockInfrRed(kobuki_msgs::DockInfraRedPtr dockInfrRedData)
	{
		...
		shared_ptr<kobuki_msgs::DockInfraRed> dockInfrRedPtr = shared_ptr<kobuki_msgs::DockInfraRed>(
				dockInfrRedData.get(), [dockInfrRedData](kobuki_msgs::DockInfraRed*) mutable
				{	dockInfrRedData.reset();});
		shared_ptr<InformationElement<kobuki_msgs::DockInfraRed>> ownDockInfrRedInfo = make_shared<
				InformationElement<kobuki_msgs::DockInfraRed>>(dockInfrRedPtr, time);
