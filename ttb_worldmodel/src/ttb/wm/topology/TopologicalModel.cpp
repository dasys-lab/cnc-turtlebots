...
void TopologicalModel::readTopologyFromConfig()
{
    ...
    for (auto& roomName : *roomNames) {
        ...
        if (!room->area) {
            room->area = getAreaInternal((*sc)["TopologicalModel"]->get<std::string>("DistributedSystems.Rooms", roomName.c_str(), "area", NULL));
        }
        if (room->pois.size() == 0) {
    ...
    for (auto& doorName : *doorNames) {
        ...
        door->fromRoom = this->getRoomInternal((*sc)["TopologicalModel"]->get<std::string>("DistributedSystems.Doors", doorName.c_str(), "from", NULL));
        ;
        door->toRoom = this->getRoomInternal((*sc)["TopologicalModel"]->get<std::string>("DistributedSystems.Doors", doorName.c_str(), "to", NULL));
        ...
        door->fromPOI = getPOIInternal((*sc)["TopologicalModel"]->get<int>("DistributedSystems.Doors", doorName.c_str(), "fromPOI", NULL));
        door->toPOI = getPOIInternal((*sc)["TopologicalModel"]->get<int>("DistributedSystems.Doors", doorName.c_str(), "toPOI", NULL));
