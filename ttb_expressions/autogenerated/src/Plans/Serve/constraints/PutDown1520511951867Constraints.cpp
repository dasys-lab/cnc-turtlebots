...
namespace alicaAutogenerated
{
    ...
    void Constraint1520850075314::getConstraint(shared_ptr<ProblemDescriptor> c, shared_ptr<RunningPlan> rp)
    {
        ...
        if (dummyXVar)
        {
            ...
            constraint->setVariable(dummyXVar, entity);
        }
        else
        {
        ...
        {
            auto entity = currentTask->getInformation().entity;
            constraint->setVariable(dummyYVar, entity);
        }
        else
        {
            std::cerr << "Constraint1520438401434: Variable type of Y didn't fit!" << std::endl;
        }
        auto dummyEntityVar = std::dynamic_pointer_cast < alica::reasoner::DummyVariable > (c->getStaticVars()->at(2));
        if (dummyEntityVar)
        {
            auto entity = currentTask->getInformation().entity;
        ...
        auto dummyZVar = std::dynamic_pointer_cast < alica::reasoner::DummyVariable > (c->getStaticVars()->at(3));
        if (dummyZVar)
        {
            auto entity = currentTask->getInformation().entity;
            constraint->setVariable(dummyZVar, entity);
        }
        else
        {
