...
//        constraint->setRule(
//                "freeFairyTailBook(X) :- book(X), genre(fairyTail), bookGenre(X, fairyTail), available(X).");
        constraint->setRule(
                "freeFairyTailBook(X) :- book(X), genre(fairyTail), bookGenre(X, fairyTail), not on(X,Y) : agent(Y).");

        constraint->setProgrammSection("bookExample");
