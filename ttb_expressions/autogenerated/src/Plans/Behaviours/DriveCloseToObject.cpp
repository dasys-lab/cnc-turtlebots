...
void DriveCloseToObject::run(void *msg)
{
    ...
    {
        std::cout << "DriveCloseToObject: Unable to get solution for variable: " << this->query->getUniqueVariableStore()->getAllRep()[0]->getName()
                  << std::endl;
        return;
    }

    std::cout << "DriveCloseToObject: Solution for variable: " << this->query->getUniqueVariableStore()->getAllRep()[0]->getName() << " is: " << result[0]
              << std::endl;
    ...
    if (this->wm->robot.isCloseTo(objectPosition))
    {
        this->goalHandle.reset();
