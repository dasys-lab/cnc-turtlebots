...
/** \brief This function is used to construct a texture image which is needed to hide a marker from the original video frame. See \e SampleMarkerHide.cpp for
 * example implementation. \param image		Pointer to the original video frame from where the hiding texture is calculated. \param hide_texture
 * Pointer to the destination image where the resulting texture is stored. \param cam			Camera object that is used for marker tracking. \param
 * gl_modelview	Current model view matrix. \param topleft		Top left limit of the texture area in marker coordinate frame.
...
/** \brief Draws the texture generated by \e BuildHideTexture to given video frame. For better performance, use OpenGL instead. See \e SampleMarkerHide.cpp for
 * example implementation. \param image		Pointer to the destination image where the texture is drawn.
