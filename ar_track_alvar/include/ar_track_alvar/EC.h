...
public:
    ...
    TrackerFeaturesEC(
            ...
            : TrackerFeatures(_max_features, _min_features, _quality_level, _min_distance, _pyr_levels, win_size)
            , purge(false)
    {
    }
...
public:
    ...
    template <typename T>
    float Reproject(std::map<int, T>& container, Pose* pose, int type_id = -1, bool needs_has_p2d = false, bool needs_has_p3d = false,
...
public:
    int GetId(int marker_id, int corner_id, int first_id = 0, int last_id = 65535) { return MarkerIdToContainerId(marker_id, corner_id, first_id, last_id); }

    /** \brief Detect markers in the image and fill in their 2D-positions in the given external container */
    template <typename T>
    int Detect(IplImage* image, Camera* cam, std::map<int, T>& container, int type_id = 0, int first_id = 0, int last_id = 65535, bool track = false,
            bool visualize = false, double max_new_marker_error = 0.08, double max_track_error = 0.2, LabelingMethod labeling_method = CVSEQ)
    {
        int ret;
