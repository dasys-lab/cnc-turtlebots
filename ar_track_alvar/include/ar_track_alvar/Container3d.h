...
public:
    ...
    bool operator()(size_t i1, size_t i2) { return (container[i1].second->size() > container[i2].second->size()); }
};
...
public:
    ...
    Iterator begin() { return Iterator(this, search_space.begin()); }

    /** \brief Provides an iterator pointing to the end of the limited/sorted 3D content */
    Iterator end() { return Iterator(this, search_space.end()); }
    ...
    size_t GetIndex(Iterator& iter) { return iter.GetIndex(); }

    /** \brief Instead of \e Iterator we can use also absolute references for data with \e operator[]() */
    node_type& operator[](size_t index) { return data[index]; }
