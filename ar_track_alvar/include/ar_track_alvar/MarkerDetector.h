...
public:
     ...
     * \param _res The marker content resolution in pixels. By default we use 5x5 markers. If you use 0 with \e MarkerData, the marker resolution is detected
     * automatically. \param _margin The marker margin resolution in pixels (The actual captured marker image has pixel resolution of _margin+_res+_margin)
     *
     * \note The default marker content resolution (_res) of 5 can only detect marker ids from 0 to 255. For larger marker ids, you need to increase the marker
     * content resolution accordingly.
     */
    void SetMarkerSize(double _edge_length = 1, int _res = 5, double _margin = 1);
     ...
     */
    int Detect(IplImage* image, Camera* cam, bool track = false, bool visualize = false, double max_new_marker_error = 0.08, double max_track_error = 0.2,
...
{
protected:
    Marker* new_M(double _edge_length = 0, int _res = 0, double _margin = 0) { return new M(_edge_length, _res, _margin); }
