...
int MarkerDetectorImpl::Detect(IplImage* image, Camera* cam, bool track, bool visualize, double max_new_marker_error, double max_track_error,
        LabelingMethod labeling_method, bool update_pose)
{
    ...
    for (size_t i = 0; i < blob_corners.size(); ++i) {
        ...
        if (ub && db && (mn->GetError(Marker::MARGIN_ERROR | Marker::DECODE_ERROR) <= max_new_marker_error)) {
