...
RansacImpl::RansacImpl(int min_params, int max_params, int sizeof_param, int sizeof_model)
{
    ...
    if (!hypothesis) {
#ifdef TRACE
        printf("Cound not allocate %d bytes of memory for model parameters.", sizeof_model);
#endif
    }
}
...
int RansacImpl::_estimate(void* params, int param_c, int support_limit, int max_rounds, void* model)
{
...
int RansacImpl::_refine(void* params, int param_c, int support_limit, int max_rounds, void* model, char* inlier_mask)
{
    if (param_c < min_params)
        return 0;
...
int RansacImpl::_estimate(int param_c, int support_limit, int max_rounds, void* model)
{
    if (param_c < min_params)
        return 0;
...
int RansacImpl::_refine(int param_c, int support_limit, int max_rounds, void* model, char* inlier_mask)
{
    if (param_c < min_params)
        return 0;
...
int RansacImpl::estimateRequiredRounds(float success_propability, float inlier_percentage)
{
    return (int) (log(1 - success_propability) / log(1 - pow(inlier_percentage, 3)));
}
