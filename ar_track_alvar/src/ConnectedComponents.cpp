...
void LabelingCvSeq::LabelSquares(IplImage* image, bool visualize)
{
    ...
    cvFindContours(bw, storage, &contours, sizeof(CvContour), CV_RETR_LIST, CV_CHAIN_APPROX_NONE, cvPoint(0, 0));
    ...
    while (contours) {
        ...
        CvSeq* result = cvApproxPoly(contours, sizeof(CvContour), storage, CV_POLY_APPROX_DP, cvContourPerimeter(contours) * 0.035, 0); // TODO: Parameters?
    ...
    for (int i = 0; i < _n_blobs; ++i) {
        ...
        if (visualize) {
            for (size_t j = 0; j < 4; ++j) {
                ...
                if (j == 0)
                    cvCircle(image, cvPoint(int(intc.x), int(intc.y)), 5, CV_RGB(255, 255, 255));
                if (j == 1)
                    cvCircle(image, cvPoint(int(intc.x), int(intc.y)), 5, CV_RGB(255, 0, 0));
                if (j == 2)
                    cvCircle(image, cvPoint(int(intc.x), int(intc.y)), 5, CV_RGB(0, 255, 0));
                if (j == 3)
                    cvCircle(image, cvPoint(int(intc.x), int(intc.y)), 5, CV_RGB(0, 0, 255));
            }
        }
    }
...
CvSeq* LabelingCvSeq::LabelImage(IplImage* image, int min_size, bool approx)
{
    ...
    while (contours) {
        ...
        if (approx) {
            CvSeq* result = cvApproxPoly(contours, sizeof(CvContour), storage, CV_POLY_APPROX_DP, cvContourPerimeter(contours) * 0.02, 0); // TODO: Parameters?
