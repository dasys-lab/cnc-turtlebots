...
FernClassifierWrapper::FernClassifierWrapper(const vector<vector<Point2f>>& points, const vector<Mat>& referenceImages, const vector<vector<int>>& labels,
        int _nclasses, int _patchSize, int _signatureSize, int _nstructs, int _structSize, int _nviews, int _compressionMethod,
        const PatchGenerator& patchGenerator)
        : FernClassifier(
                  points, referenceImages, labels, _nclasses, _patchSize, _signatureSize, _nstructs, _structSize, _nviews, _compressionMethod, patchGenerator)
{
}
...
FernImageDetector::FernImageDetector(const bool visualize)
        : mPatchGenerator(0, 256, 13, true, /*0.25*/ 0.10, 1.0 /*0.6, 1.5*/, -CV_PI * 1.0, CV_PI * 1.0, -CV_PI * 0.0,
                  CV_PI * 0.0 /*-2*CV_PI, 2*CV_PI*/) // TODO: check angle values, cant be -2pi..2pi ?
...
void FernImageDetector::train(Mat& object)
{
    ...
    mClassifier[0].trainFromSingleView(blurredObject, mKeyPoints, PATCH_SIZE, SIGNATURE_SIZE,
    ...
    if (planeAssumption && fromPt.size() > 8) {
        ...
        for (size_t i = 0, j = 0; i < (int) pairs.size(); i += 2, ++j) {
            if (mask[j]) {
                ...
                line(mCorrespondences, mKeyPoints[pairs[i]].pt, keypoints[pairs[i + 1]].pt + Point2f(0.0, (float) mObjects[0].rows),
