...
UnscentedKalman::UnscentedKalman(int state_n, int obs_n, int state_k, double alpha, double beta)
{
    ...
    state = cvCreateMat(state_n, 1, CV_64F);
    cvSetZero(state);
    stateCovariance = cvCreateMat(state_n, state_n, CV_64F);
    cvSetZero(stateCovariance);
    sqrtStateCovariance = cvCreateMat(state_n, state_n, CV_64F);
    cvSetZero(sqrtStateCovariance);
    stateD = cvCreateMat(state_n, state_n, CV_64F);
    cvSetZero(stateD);
    stateU = cvCreateMat(state_n, state_n, CV_64F);
    cvSetZero(stateU);
    stateV = cvCreateMat(state_n, state_n, CV_64F);
    cvSetZero(stateV);
    stateTmp = cvCreateMat(state_n, state_n, CV_64F);
    cvSetZero(stateTmp);
    stateDiff = cvCreateMat(state_n, 1, CV_64F);
    cvSetZero(stateDiff);

    predObs = cvCreateMat(obs_n, 1, CV_64F);
    cvSetZero(predObs);
    predObsCovariance = cvCreateMat(obs_n, obs_n, CV_64F);
    cvSetZero(predObsCovariance);
    predObsDiff = cvCreateMat(obs_n, 1, CV_64F);
    cvSetZero(predObsDiff);

    invPredObsCovariance = cvCreateMat(obs_n, obs_n, CV_64F);
    cvSetZero(invPredObsCovariance);
    statePredObsCrossCorrelation = cvCreateMat(state_n, obs_n, CV_64F);
    cvSetZero(statePredObsCrossCorrelation);
    kalmanGain = cvCreateMat(state_n, obs_n, CV_64F);
    cvSetZero(kalmanGain);
    kalmanTmp = cvCreateMat(state_n, obs_n, CV_64F);
    cvSetZero(kalmanTmp);
    ...
    for (int i = 0; i < sigma_n; i++) {
        sigma_state[i] = cvCreateMat(state_n, 1, CV_64F);
        cvSetZero(sigma_state[i]);
        sigma_predObs[i] = cvCreateMat(obs_n, 1, CV_64F);
        cvSetZero(sigma_predObs[i]);
    }
...
void UnscentedKalman::initialize()
{
    ...
    for (int i = 0; i < 5; i++)
        printf("%+.10f %+.10f %+.10f %+.10f %+.10f\n", cvGetReal2D(stateCovariance, 0, i), cvGetReal2D(stateCovariance, 1, i),
                cvGetReal2D(stateCovariance, 2, i), cvGetReal2D(stateCovariance, 3, i), cvGetReal2D(stateCovariance, 4, i));
#endif
    ...
    for (int i = 0; i < 5; i++)
        printf("%+.10f %+.10f %+.10f %+.10f %+.10f\n", cvGetReal2D(sqrtStateCovariance, 0, i), cvGetReal2D(sqrtStateCovariance, 1, i),
                cvGetReal2D(sqrtStateCovariance, 2, i), cvGetReal2D(sqrtStateCovariance, 3, i), cvGetReal2D(sqrtStateCovariance, 4, i));
    cvGEMM(sqrtStateCovariance, sqrtStateCovariance, 1., NULL, 0, stateTmp);
    printf("sqrtStateCovariance^2:\n");
    for (int i = 0; i < 5; i++)
        printf("%+.10f %+.10f %+.10f %+.10f %+.10f\n", cvGetReal2D(stateTmp, 0, i), cvGetReal2D(stateTmp, 1, i), cvGetReal2D(stateTmp, 2, i),
                cvGetReal2D(stateTmp, 3, i), cvGetReal2D(stateTmp, 4, i));
#endif
...
void UnscentedKalman::predict(UnscentedProcess* process_model)
{
    ...
    for (int i = 0; i < sigma_n; i++) {
        ...
        cvGEMM(stateDiff, stateDiff, scale, stateCovariance, 1., stateCovariance, CV_GEMM_B_T);
    }

    // Add any additive noise.
    CvMat* noise = process_model->getProcessNoise();
    if (noise)
        cvAdd(stateCovariance, noise, stateCovariance);
    ...
    for (int i = 0; i < state_n; i++) {
        for (int j = 0; j < state_n; j++)
            printf("%+f ", cvGetReal2D(stateCovariance, i, j));
        printf("\n");
    }
#endif
...
void UnscentedKalman::update(UnscentedObservation* obs)
{
    ...
    for (int i = 0; i < sigma_n; i++) {
        ...
        double scale = i == 0 ? (double) state_k / (double) (state_n + state_k) : .5 / (double) (state_n + state_k);
        obs->h(&sigma_h, sigma_state[i]);
        cvAddWeighted(&sigma_h, scale, &predObs, 1., 0., &predObs);
    }

    // Compute predicted observation co-variance.
    CvMat predObsCovariance = cvMat(obs_n, obs_n, CV_64F, this->predObsCovariance->data.db);
    CvMat statePredObsCrossCorrelation = cvMat(state_n, obs_n, CV_64F, this->statePredObsCrossCorrelation->data.db);
    ...
    for (int i = 0; i < sigma_n; i++) {
        ...
        cvGEMM(&predObsDiff, &predObsDiff, scale, &predObsCovariance, 1., &predObsCovariance, CV_GEMM_B_T);
        cvGEMM(stateDiff, &predObsDiff, scale, &statePredObsCrossCorrelation, 1., &statePredObsCrossCorrelation, CV_GEMM_B_T);
    }

    // Add any additive noise.
    CvMat* noise = obs->getObservationNoise();
    if (noise)
        cvAdd(&predObsCovariance, noise, &predObsCovariance);
    ...
    for (int i = 0; i < obs_n; i++) {
        for (int j = 0; j < obs_n; j++)
            printf("%+f ", cvGetReal2D(&predObsCovariance, i, j));
        printf("\n");
    }
    printf("state observation cross-correlation\n");
    for (int i = 0; i < state_n; i++) {
        for (int j = 0; j < obs_n; j++)
            printf("%+f ", cvGetReal2D(&statePredObsCrossCorrelation, i, j));
        printf("\n");
    }
#endif
    ...
    CvMat invPredObsCovariance = cvMat(obs_n, obs_n, CV_64F, this->invPredObsCovariance->data.db);
    ...
    cvGEMM(&kalmanTmp, &kalmanGain, -1., stateCovariance, 1., stateCovariance, CV_GEMM_B_T);
#ifdef MYDEBUG
