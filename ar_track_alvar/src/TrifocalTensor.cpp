    ...
    for (int i = 0; i < 3; i++)
        for (int j = 0; j < 3; j++)
            for (int k = 0; k < 3; k++) {
                ...
                T[i][j][k] = (data_p12[j][i] * data_p13[k][3]) - (data_p12[j][3] * data_p13[k][i]);
            }
}
...
void TrifocalTensor::computeTensor(const Pose& p0, const Pose& p1, const Pose& p2)
{
    ...
    for (int i = 0; i < 3; i++)
        for (int j = 0; j < 3; j++)
            for (int k = 0; k < 3; k++) {
                double sign = i == 1 ? -1 : 1;
                T[i][j][k] = sign * det(getRow(data_p0, (i + 1) % 3), getRow(data_p0, (i + 2) % 3), getRow(data_p1, j), getRow(data_p2, k));
            }
}

double TrifocalTensor::projectAxis(const CvPoint2D64f& p0, const CvPoint2D64f& p1, int l)
{
    double v00 = p1.x * (p0.x * T[0][0][l] + p0.y * T[1][0][l] + T[2][0][l]) - p1.x * (p0.x * T[0][0][l] + p0.y * T[1][0][l] + T[2][0][l]);
    double v01 = p1.x * (p0.x * T[0][1][l] + p0.y * T[1][1][l] + T[2][1][l]) - p1.y * (p0.x * T[0][0][l] + p0.y * T[1][0][l] + T[2][0][l]);
    double v02 = p1.x * (p0.x * T[0][2][l] + p0.y * T[1][2][l] + T[2][2][l]) - (p0.x * T[0][0][l] + p0.y * T[1][0][l] + T[2][0][l]);

    double v10 = p1.y * (p0.x * T[0][0][l] + p0.y * T[1][0][l] + T[2][0][l]) - p1.x * (p0.x * T[0][1][l] + p0.y * T[1][1][l] + T[2][1][l]);
    double v11 = p1.y * (p0.x * T[0][1][l] + p0.y * T[1][1][l] + T[2][1][l]) - p1.y * (p0.x * T[0][1][l] + p0.y * T[1][1][l] + T[2][1][l]);
    double v12 = p1.y * (p0.x * T[0][2][l] + p0.y * T[1][2][l] + T[2][2][l]) - (p0.x * T[0][1][l] + p0.y * T[1][1][l] + T[2][1][l]);

    double v20 = (p0.x * T[0][0][l] + p0.y * T[1][0][l] + T[2][0][l]) - p1.x * (p0.x * T[0][2][l] + p0.y * T[1][2][l] + T[2][2][l]);
    double v21 = (p0.x * T[0][1][l] + p0.y * T[1][1][l] + T[2][1][l]) - p1.y * (p0.x * T[0][2][l] + p0.y * T[1][2][l] + T[2][2][l]);

    double v22 = (p0.x * T[0][2][l] + p0.y * T[1][2][l] + T[2][2][l]) - (p0.x * T[0][2][l] + p0.y * T[1][2][l] + T[2][2][l]);
