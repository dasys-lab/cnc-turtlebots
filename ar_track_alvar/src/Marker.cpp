...
void Marker::VisualizeMarkerPose(IplImage* image, Camera* cam, double visualize2d_points[12][2], CvScalar color) const
{
    // Cube
    for (int i = 0; i < 4; i++) {
        cvLine(image, cvPoint((int) visualize2d_points[i][0], (int) visualize2d_points[i][1]),
                cvPoint((int) visualize2d_points[(i + 1) % 4][0], (int) visualize2d_points[(i + 1) % 4][1]), color);
        cvLine(image, cvPoint((int) visualize2d_points[i][0], (int) visualize2d_points[i][1]),
                cvPoint((int) visualize2d_points[4 + i][0], (int) visualize2d_points[4 + i][1]), color);
        cvLine(image, cvPoint((int) visualize2d_points[4 + i][0], (int) visualize2d_points[4 + i][1]),
                cvPoint((int) visualize2d_points[4 + ((i + 1) % 4)][0], (int) visualize2d_points[4 + ((i + 1) % 4)][1]), color);
    }
    // Coordinates
    cvLine(image, cvPoint((int) visualize2d_points[8][0], (int) visualize2d_points[8][1]),
            cvPoint((int) visualize2d_points[9][0], (int) visualize2d_points[9][1]), CV_RGB(255, 0, 0));
    cvLine(image, cvPoint((int) visualize2d_points[8][0], (int) visualize2d_points[8][1]),
            cvPoint((int) visualize2d_points[10][0], (int) visualize2d_points[10][1]), CV_RGB(0, 255, 0));
    cvLine(image, cvPoint((int) visualize2d_points[8][0], (int) visualize2d_points[8][1]),
            cvPoint((int) visualize2d_points[11][0], (int) visualize2d_points[11][1]), CV_RGB(0, 0, 255));
}
    ...
    for (int j = 0; j < marker_content->height; j++) {
        for (int i = 0; i < marker_content->width; i++) {
            ...
            */
            ...
            if (marker_points_img[(j * marker_content->width) + i].val > max)
                max = marker_points_img[(j * marker_content->width) + i].val;
            if (marker_points_img[(j * marker_content->width) + i].val < min)
                min = marker_points_img[(j * marker_content->width) + i].val;
        }
    }
...
void Marker::ScaleMarkerToImage(IplImage* image) const
{
    ...
    CvMat submat;
    cvGetSubRect(
            img, &submat, cvRect(int(multiplier * margin + 0.5), int(multiplier * margin + 0.5), int(multiplier * res + 0.5), int(multiplier * res + 0.5)));
    cvResize(marker_content, img_content, CV_INTER_NN);
    cvCopy(img_content, &submat);
    ...
    for (int j = 0; j < res; j++) {
        for (int i = 0; i < res; i++) {
            if (*orientation == 0) {
                ...
                if ((j == res - 1) && (i == res - 1))
                    continue;
                if (cvGetReal2D(marker_content, j, i))
                    bs.push_back(false);
                else
                    bs.push_back(true);
                ...
                if (((res - i - 1) == 0) && (j == res - 1))
                    continue;
                if (cvGetReal2D(marker_content, res - i - 1, j))
                    bs.push_back(false);
                else
                    bs.push_back(true);
                ...
                if (((res - j - 1) == 0) && ((res - i - 1) == 0))
                    continue;
                if (cvGetReal2D(marker_content, res - j - 1, res - i - 1))
                    bs.push_back(false);
                else
                    bs.push_back(true);
                ...
                if ((i == res - 1) && ((res - j - 1) == 0))
                    continue;
                if (cvGetReal2D(marker_content, i, res - j - 1))
                    bs.push_back(false);
                else
                    bs.push_back(true);
            }
...
void MarkerData::DecodeOrientation(int* error, int* total, int* orientation)
{
    ...
    for (i = 0; i < res; i++) {
        (*total)++;
        if ((int) cvGetReal2D(marker_content, j, i) != color)
            errors[0]++;
        if ((int) cvGetReal2D(marker_content, i, j) != color)
            errors[1]++;
        color = (color ? 0 : 255);
    }
    errors[2] = errors[0];
    ...
    for (j = (res / 2) - 2; j <= (res / 2) + 2; j++) {
        if (j < (res / 2)) {
            (*total)++;
            if ((int) cvGetReal2D(marker_content, j, i) != 0)
                errors[0]++;
            if ((int) cvGetReal2D(marker_content, i, j) != 0)
                errors[1]++;
            if ((int) cvGetReal2D(marker_content, j, i) != 255)
                errors[2]++;
            if ((int) cvGetReal2D(marker_content, i, j) != 255)
                errors[3]++;
        } else if (j > (res / 2)) {
            (*total)++;
            if ((int) cvGetReal2D(marker_content, j, i) != 255)
...
bool MarkerData::DetectResolution(vector<PointDouble>& _marker_corners_img, IplImage* gray, Camera* cam)
{
    ...
    if ((pt2.x < 0) || (pt2.y < 0) || (pt2.x >= gray->width) || (pt2.y >= gray->height)) {
        return false;
    }
    bool white = true;
    for (int i = 0; i < 4; i++) {
        ...
        if ((pt1.x < 0) || (pt1.y < 0) || (pt1.x >= gray->width) || (pt1.y >= gray->height)) {
            return false;
        }
...
int MarkerData::DecodeCode(int orientation, BitsetExt* bs, int* erroneous, int* total, unsigned char* content_type)
{
    ...
    for (int j = 0; j < res; j++) {
        for (int i = 0; i < res; i++) {
            ...
            int color = 0;
            if (orientation == 0)
                color = (int) cvGetReal2D(marker_content, j, i);
            else if (orientation == 1)
                color = (int) cvGetReal2D(marker_content, res - i - 1, j);
            else if (orientation == 2)
                color = (int) cvGetReal2D(marker_content, res - j - 1, res - i - 1);
            else if (orientation == 3)
                color = (int) cvGetReal2D(marker_content, i, res - j - 1);
            if (color)
                bs->push_back(false);
            else
                bs->push_back(true);
