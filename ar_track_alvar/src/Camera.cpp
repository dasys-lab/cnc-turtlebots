...
bool ProjPoints::AddPointsUsingChessboard(IplImage* image, double etalon_square_size, int etalon_rows, int etalon_columns, bool visualize)
{
    ...
    if (!pattern_was_found)
        point_count = 0;
    if (point_count > 0) {
        cvFindCornerSubPix(gray, corners, point_count, cvSize(5, 5), cvSize(-1, -1), cvTermCriteria(CV_TERMCRIT_ITER | CV_TERMCRIT_EPS, 10, 0.01f));
    ...
    if (visualize) {
        cvDrawChessboardCorners(image, cvSize(etalon_rows, etalon_columns), corners, point_count, false /*pattern_was_found*/);
    }
    delete[] corners;
    cvReleaseImage(&gray);
    if (point_count > 0)
...
bool ProjPoints::AddPointsUsingMarkers(vector<PointDouble>& marker_corners, vector<PointDouble>& marker_corners_img, IplImage* image)
{
    width = image->width;
    height = image->height;
    if ((marker_corners.size() == marker_corners_img.size()) && (marker_corners.size() == 4)) {
...
Camera::Camera(ros::NodeHandle& n, std::string cam_info_topic)
        : n_(n)
{
    calib_K = cvMat(3, 3, CV_64F, calib_K_data);
    calib_D = cvMat(4, 1, CV_64F, calib_D_data);
...
bool Camera::LoadCalibXML(const char* calibfile)
{
    ...
    return xml_root->QueryIntAttribute("width", &calib_x_res) == TIXML_SUCCESS && xml_root->QueryIntAttribute("height", &calib_y_res) == TIXML_SUCCESS &&
...
void Camera::Calibrate(ProjPoints& pp)
{
    ...
    cvCalibrateCamera2(object_points, image_points, &point_counts, cvSize(pp.width, pp.height), &calib_K, &calib_D, 0, 0, CV_CALIB_USE_INTRINSIC_GUESS);
...
void Camera::GetOpenglProjectionMatrix(
...
void Camera::CalcExteriorOrientation(vector<CvPoint3D64f>& pw, vector<CvPoint2D64f>& pi, Pose* pose)
{
...
void Camera::CalcExteriorOrientation(vector<CvPoint3D64f>& pw, vector<PointDouble>& pi, CvMat* rodriques, CvMat* tra)
{
...
void Camera::CalcExteriorOrientation(vector<PointDouble>& pw, vector<PointDouble>& pi, CvMat* rodriques, CvMat* tra)
{
    // assert(pw.size() == pi.size());
    int size = (int) pi.size();
...
void Camera::ProjectPoints(const CvMat* object_points, const CvMat* rotation_vector, const CvMat* translation_vector, CvMat* image_points) const
{
    // Project points
