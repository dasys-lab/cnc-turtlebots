...
{
    if (marker_id == 0) {
        if (marker_status[get_id_index(marker_id)] == 0)
            PointCloudAdd(marker_id, edge_length, pose);
    } else {
...
double MultiMarkerFiltered::_Update(MarkerIterator& begin, MarkerIterator& end, Camera* cam, Pose& pose, IplImage* image)
{
    if (_GetPose(begin, end, cam, pose, NULL) == -1)
        return -1;
