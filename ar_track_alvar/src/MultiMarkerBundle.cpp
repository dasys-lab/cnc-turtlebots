...
void Est(CvMat* state, CvMat* estimation, void* param)
{
    ...
    for (int i = 0; i < n_images; ++i) {
        ...
        for (int j = 0; j < n_points; ++j) {
            ...
            double object_points[3] = {state->data.db[index + 0], state->data.db[index + 1], state->data.db[index + 2]};
            ...
            cvProjectPoints2(&mat_object_points, &mat_rotation_vector, &mat_translation_vector, &(camera->calib_K), &(camera->calib_D), &mat_proj);
...
bool MultiMarkerBundle::Optimize(Camera* _cam, double stop, int max_iter, Optimization::OptimizeMethod method)
{
    ...
    optimization_error = optimization.Optimize(parameters_mat, measurements_mat, stop, max_iter, Est, 0, method, parameters_mask_mat, NULL, weight_mat);
...
void MultiMarkerBundle::_MeasurementsAdd(MarkerIterator& begin, MarkerIterator& end, const Pose& camera_pose)
{
    ...
    for (MarkerIterator& i = begin.reset(); i != end; ++i) {
        ...
        // cout<<"Id "<<id<<" ";
        for (int j = 0; j < 4; j++) {
            measurements[measurements_index(frame_no, id, j)] = marker->marker_corners_img[j];
