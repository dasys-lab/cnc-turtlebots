...
void TrackerFeatures::ChangeSettings(int _max_features, int _min_features, double _quality_level, double _min_distance)
{
    ...
    if (_max_features == max_features && _min_features == min_features && _quality_level == quality_level && _min_distance == min_distance)
        return;
...
double TrackerFeatures::TrackHid(IplImage* img, IplImage* new_features_mask, bool add_features)
{
    ...
    if (frame_count <= 1) {
        ...
        // Track
        cvCalcOpticalFlowPyrLK(prev_gray, gray, prev_pyramid, pyramid, prev_features, features, prev_feature_count, cvSize(win_size, win_size), pyr_levels,
                status, 0, cvTermCriteria(CV_TERMCRIT_ITER | CV_TERMCRIT_EPS, 20, 0.03), 0);
        feature_count = 0;
...
IplImage* TrackerFeatures::NewFeatureMask()
{
    ...
    for (int i = 0; i < feature_count; i++) {
        cvRectangle(mask, cvPoint(int(features[i].x - min_distance), int(features[i].y - min_distance)),
                cvPoint(int(features[i].x + min_distance), int(features[i].y + min_distance)), cvScalar(0), CV_FILLED);
    }
    return mask;
}
...
int TrackerFeatures::AddFeatures(IplImage* new_features_mask)
{
    ...
    if (feature_count < min_features) {
        ...
        if (new_features_mask == NULL) {
            ...
            // Find new features
            cvGoodFeaturesToTrack(gray, img_eig, img_tmp, &(features[feature_count]), &new_feature_count, quality_level, min_distance, mask, 3, 1, 0.04);

        } else {
            cvGoodFeaturesToTrack(
                    gray, img_eig, img_tmp, &(features[feature_count]), &new_feature_count, quality_level, min_distance, new_features_mask, 3, 1, 0.04);
        }
