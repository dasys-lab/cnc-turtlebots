...
bool TrackerOrientation::UpdatePose(IplImage* image)
{
    ...
    for (map<int, Feature>::iterator it = _F_v.begin(); it != _F_v.end(); it++) {
        if ((it->second.status3D == Feature::USE_FOR_POSE || it->second.status3D == Feature::IS_INITIAL) && it->second.status2D == Feature::IS_TRACKED) {
    ...
    double rot[3];
    CvMat rotm = cvMat(3, 1, CV_64F, rot);
...
bool TrackerOrientation::UpdateRotationOnly(IplImage* gray, IplImage* image)
{
    if (gray->nChannels != 1)
        return false;
    if (!_grsc)
        _grsc = cvCreateImage(cvSize(_xres, _yres), 8, 1);
