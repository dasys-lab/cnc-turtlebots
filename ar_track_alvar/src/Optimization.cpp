...
{
    estimate_param = 0;
    J = cvCreateMat(n_meas, n_params, CV_64F);
    cvZero(J);
    JtJ = cvCreateMat(n_params, n_params, CV_64F);
    cvZero(JtJ);
    tmp = cvCreateMat(n_params, n_meas, CV_64F);
    cvZero(tmp);
    W = cvCreateMat(n_meas, n_meas, CV_64F);
    cvZero(W);
    diag = cvCreateMat(n_params, n_params, CV_64F);
    cvZero(diag);
    err = cvCreateMat(n_meas, 1, CV_64F);
    cvZero(err);
    delta = cvCreateMat(n_params, 1, CV_64F);
    cvZero(delta);
    x_minus = cvCreateMat(n_params, 1, CV_64F);
    cvZero(x_minus);
    x_plus = cvCreateMat(n_params, 1, CV_64F);
    cvZero(x_plus);
    x_tmp1 = cvCreateMat(n_meas, 1, CV_64F);
    cvZero(x_tmp1);
    x_tmp2 = cvCreateMat(n_meas, 1, CV_64F);
    cvZero(x_tmp2);
    tmp_par = cvCreateMat(n_params, 1, CV_64F);
    cvZero(tmp_par);
}
...
double Optimization::Optimize(CvMat* parameters, // Initial values are set
        ...
        double stop, int max_iter, EstimateCallback Estimate, void* param, OptimizeMethod method,
        CvMat* parameters_mask, // Mask indicating non-constant parameters)
        CvMat* J_mat, CvMat* weights)
{
